...
LSTM_LABEL_PORT=int(Config['lstm_label']['port'])
...
class Model_LSTM(object):
    ...
    def infer(self,dd,sent,max_label,sess):
        '''
        ...
        for ele in intent_logit:
            ...
            ss=[[e[0],str(e[1])] for e in ss]
            res.append(ss[:max_label])
        return res
...
def main(_):
    ...
    with tf.device("/cpu:1"):
        ...
        nn_model = Model_LSTM(slot_num_class=dd.slot_num, intent_num_class=dd.intent_num, vocab_num=dd.vocab_num)
        if FLAGS.mod == 'train':
            nn_model.train(dd)
            ...
            def intent(sent_list):
                ...
                for sent in sent_list:
                    ...
                    sents.append(sent)
                all_res = nn_model.infer(dd, sents,10,sess)
                del sents
                gc.collect()
