...
class Intent_Slot_Data(object):
    ...
    intent_slot 数据处理模块
    '''
    def __init__(self, train_path, dev_path, test_path, batch_size, flag,max_length,use_auto_bucket,max_intent_length):
        ...
        self.index=0
        if flag == "train_new":
            self.vocab, self.slot_vocab, self.intent_vocab,self.intent_word_vocab = self.get_vocab()
            ...
            pickle.dump(self.intent_word_vocab, open(PATH + "/intent_word_vocab.p", 'wb'))  # 词典
            ...
            self.intent_word_vocab = pickle.load(open(PATH + "/intent_word_vocab.p", 'rb'))  # 词典
        ...
        self.intent_word_num=len(self.intent_word_vocab)
        ...
        self.intent_words,self.intent_real_len=self.get_intent_word_array()


    def get_intent_word_array(self):
        word_arr=[]
        real_len_arr=[]
        for i in range(self.intent_num):
            ...
            ss=[]
            for w in k:
                if w not in self.intent_word_vocab:
                    ss.append(self.intent_word_vocab['none'])
                else:
                    ss.append(self.intent_word_vocab[w])
            real_len_arr.append(len(ss))
            if len(ss)>=self.max_intent_length:
                ss=ss[:self.max_intent_length]
            else:
                ss.extend([0]*(self.max_intent_length-len(ss)))
            word_arr.append(ss)

        return np.array(word_arr),np.array(real_len_arr)
    ...
    def get_vocab(self):
        ...
        def _vocab(file):
            for ele in file:
                ...
                intents=[e for e in eles[2].strip().split(' ')]
                ...
                for intent in intents:
                    ...
                    for word in intent:
                        if word not in intent_word_vocab:
                            intent_word_vocab[word]=self.intent_word_index
                            self.intent_word_index+=1
        ...
        return vocab,slot_vocab,intent_vocab,intent_word_vocab
    ...
    def padd_sentences_no_buckets(self, sent_list):
        ...
        for sent in sent_list:
            ...
            words.append(sents[0].split(' '))
            slot_labels.append(sents[1].split(' '))
            intent_labels.append(sents[2].strip().split(' '))
    ...
    def get_sent_char(self,sent_list):
        ...
        if self.use_auto_bucket:
            data_list=[]
            for sent in sent_list:
                ss_ls=[e for e in sent.split(' ')]
                ss_ = ['bos'].extend(ss_ls)
                sent = [e.lower() for e in jieba.cut(sent)]
                sent.append('eos')
                print(sent)
                ss = []
...
if __name__ == '__main__':
    ...
    dd = Intent_Slot_Data(train_path="../dataset/train_out_char.txt",
                              ...
                              dev_path="../dataset/dev_out_char.txt", batch_size=20 ,max_length=30,max_intent_length=16,flag="train_new",use_auto_bucket=False)
