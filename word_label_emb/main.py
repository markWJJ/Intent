...
from word_label_emb.model1 import *
import numpy as np
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        for _ in range(100):
            ...
            train_soft_logit_, train_loss_, _ = sess.run([soft_logit, loss, optimizer], feed_dict={sent_word: train_sent,
                                                                                       sent_len: train_rel_len,
                                                                                       intent_y: train_intent
                                                                                       })
            acc=intent_acc(train_soft_logit_,train_intent)
            print(train_loss_,acc)
