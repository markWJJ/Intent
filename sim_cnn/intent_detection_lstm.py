...
class Model_LSTM(object):
    ...
    def __init__(self,intent_num_class,vocab_num,intent_words,intent_rel_len,intent_word_num):
    ...
    def init_graph(self):
        ...
        self.intent_embedding=tf.Variable(tf.random_normal(shape=(self.intent_num_class,300),dtype=tf.float32))
    ...
    def intent_loss(self):
        ...
        # lstm_w = tf.Variable(tf.random_normal(shape=(4 * self.hidden_dim, 300), dtype=tf.float32))
        # tf.add_to_collection('l2', tf.contrib.layers.l2_regularizer(FLAGS.lambda1)(lstm_w))
        # lstm_b = tf.Variable(tf.random_normal(shape=(300,), dtype=tf.float32))
        ...
        intent_out = tf.layers.dense(self.intent_embedding, 300, activation=None)
        lstm_out = tf.layers.dense(lstm_out, 300, activation=None)
        sent_emb_norm = tf.sqrt(tf.reduce_sum(tf.square(lstm_out), axis=1))  # 模
        ...
        for ele in intent_norms:
            ...
            sent_intent = tf.matmul(lstm_out, ele)
...
def main(_):
    ...
    nn_model = Model_LSTM(intent_num_class=dd.intent_num, vocab_num=dd.vocab_num, intent_words=dd.intent_words,
                     intent_rel_len=dd.intent_real_len,
